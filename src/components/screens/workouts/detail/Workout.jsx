	...
	return (
		<>
			...
			<div className={`wrapper-inner-page ${styles['wrapper-inner-page-bottom']}`}>
				<div style={{ width: '90%', margin: '0 auto' }}>{/* {errorCompleted && <Alert type='error' text={errorCompleted} />} */}</div>
				{isLoading ? (
					<Loader />
				) : (
					<div className={styles.wrapper}>
						{workoutLog?.exerciseLogs?.map((exerciseLog, index) => (
							<Fragment key={exerciseLog.id}>
								...
								{index % 2 !== 0 && index !== workoutLog.exerciseLogs.length - 1 && <div className={styles.line} />}
							</Fragment>
						))}
					</div>
				)}
